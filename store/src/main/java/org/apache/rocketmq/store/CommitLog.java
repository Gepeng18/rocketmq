...
public class CommitLog {
    ...
    // 可以设置使用ReentrantLock 也可以设置使用cas，默认是使用cas
    ...
    /**
     * 删除过期的文件
     * @param expiredTime 过期时间，默认是72小时
     * @param deleteFilesInterval 删除文件的间隔，默认是100s
     * @param intervalForcibly 强制删除时间，120s
    ...
    // 根据offset从mappedFile中获取数据，先找到相应的文件，再找到文件中相应位置的buffer
...
//        int queueId msg.getQueueId();
        ...
        if (tranType == MessageSysFlag.TRANSACTION_NOT_TYPE
            ...
            if (msg.getDelayTimeLevel() > 0) {
                ...
                // 其实你有没有发现，他的实现与事务消息的差不多，都是修改topic 与queueId
        ...
        try {
            ...
            // 设置获取锁的时间戳(在释放锁之前会重置成0)，主要目的是为了判断os page cache繁忙
            ...
            switch (result.getStatus()) {
                case PUT_OK:
                    break;
                case END_OF_FILE:
                    // 如果mappedFile放不下消息，就重新获取一个mappedFile，重新追加
        ...
        try {
            ...
            switch (result.getStatus()) { // 判断写入状态
                case PUT_OK:
                    break;
                case END_OF_FILE:
                    // 如果是那种MappedFile放不开消息的情况，它会重新获取一个MappedFile
                    unlockMappedFile = mappedFile;
                    // Create a new file, re-write the message
                    mappedFile = this.mappedFileQueue.getLastMappedFile(0);
                    if (null == mappedFile) {
    ...
    public CompletableFuture<PutMessageStatus> submitFlushRequest(AppendMessageResult result, MessageExt messageExt) {
        // 同步刷盘 ：如果broker配置的SYNC_FLUSH 并且是个同步消息，这个时候就会创建一个刷盘请求，然后提交刷盘请求，这个时候会等着刷盘完成，默认就是5s。
    ...
    class DefaultAppendMessageCallback implements AppendMessageCallback {
        ...
        public AppendMessageResult doAppend(final long fileFromOffset, final ByteBuffer byteBuffer, final int maxBlank,
            ...
            // 第一部分，计算了一下这个消息写在commitlog中的一个offset，接着就是生成一个msgId，
            // 然后根据topic 与queueId从缓存中获取了一下这个queueId对应的一个queue的offset，这个其实就是添加一个消息加1，
            // 然后就是事务的东西了，如果有事务，然后还在准备阶段或者回滚阶段，就将queue offset 设置成0，再往下其实就是处理消息，
            // 然后写到buffer中了。

            // PHY OFFSET
            // 1、计算总的偏移量，在整个commitLog中的一个总的
            ...
            if ((msgLen + END_FILE_MIN_BLANK_LENGTH) > maxBlank) {
                // 空间不足，就塞上一个结束的东西，8个字节是正经的，剩下的随意，然后返回文件满了的状态。
            ...
            switch (tranType) {
                ...
                case MessageSysFlag.TRANSACTION_COMMIT_TYPE:
                    ...
                    // 记录，更新下队列的一个偏移量 queue offset (其实就是+1)
                    CommitLog.this.topicQueueTable.put(key, ++queueOffset);
    ...
    public static class MessageExtEncoder {
        ...
        protected PutMessageResult encode(MessageExtBrokerInner msgInner) {
            /**
             * Serialize message
             * 获取了一下消息里面的properties，将它转成字节数组，计算了一下长度
             */
            ...
            // 接着就是将topic转成字节数据，计算了一下长度
            final byte[] topicData = msgInner.getTopic().getBytes(MessageDecoder.CHARSET_UTF8);
            final int topicLength = topicData.length;

            // 获取了一下body的长度，就是你往Message塞得内容长度
            final int bodyLength = msgInner.getBody() == null ? 0 : msgInner.getBody().length;

            // 计算这个消息需要的总长度,这里面不光 是消息本身的长度,其实还有一些系统参数,
            final int msgLen = calMsgLength(msgInner.getSysFlag(), bodyLength, topicLength, propertiesLength);

            // Exceeds the maximum message
            // 判断一下消息总长度是否超长
            if (msgLen > this.maxMessageSize) { // 最大消息65535
            ...
            // 6 QUEUEOFFSET, need update later 在队列中的一个偏移量
            this.encoderBuffer.putLong(0);
            // 7 PHYSICALOFFSET, need update later 在整个大commitlog的一个偏移量
            this.encoderBuffer.putLong(0);
            // 8 SYSFLAG  记录系统flag
            this.encoderBuffer.putInt(msgInner.getSysFlag());
            // 9 BORNTIMESTAMP  记录出生日期
            this.encoderBuffer.putLong(msgInner.getBornTimestamp());
            // 10 BORNHOST  记录出生地址，就是哪个消息生产者发过来的消息
